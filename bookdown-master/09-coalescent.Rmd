...
Let's assume we have $N$ lines in Fisher-Wright (Note: for now we are not using $2N$.) As a reference, think about the population being the dots from the last column of the diagram from the last section. Then this sample is trying to look backwards $n$ generations until they find their MRCA. 
  ...
  *With a reasonably big population and with a long time scale, we can think about the problem in continuous time* 
  ...
  - Let the hazard of coalescence = $c = 1/N$.
...
>>>>>>> 6aafe73ec628f179301b0a0fb97be1812f8569bd
...
fwm2 <- function(N, n_gen, mu = 0, start_types = paste(1:N))
{
    ...
    return(list(A=A, P=P)) ## matrix of types, each line a generation.
}
...
get.coalescence.time <- function(P, i,j, verbose = F)
{
    ...
    for (k in nrow(P):1)
    {
        ...
        if (verbose == TRUE)
            print(parents)
        parents <- P[k, parents]
        PP[k, parents] <- P[k, parents]*10 + P[k, parents]
...
get.coalescence.time(P,1,2, verbose = T)
get.coalescence.time(P,3,4, verbose = T)
## test when there's no coalescence
set.sed(10)
out <- fwm2(5,2,0)
P <- out$P
print(P)
get.coalescence.time(P,1,2, verbose = T)
get.coalescence.time(P,3,4, verbose = T)
get.coalescence.time(P,3,3, verbose = T)


## Now let's do average of coalescent time in different fw's
## we'll just draw 1 pair for each one
set.seed(2)
N <- 50 ## pop size
...
```
  ...
  - $\bar{T}$ is the average number of generations until we reach the most common recent ancestor and from the previous section we know that: $$ \bar{T} = E(TMRCA) = N $$0
...
- We said hazard of coalescence was $h = c = 1/N$.
...
```{r , fig.height = 4, fig.cap='Histogram', fig.align='center'}
  ...
  T.vec <- c(T1, T2)
  hist(T.vec, breaks = seq(0, 5000, 250), main="")
```
...
look at pairwise differences of random pairs of individuals. 

For this we are not going to do subgroups such as the Greeks or Irish, but are
...
```{r, fig.height=4, fig.align='center', fig.cap='Etimatated probability of not coalescing'}
...
**_1. From smoothed survival curve_**
...
plot(x.mid, nmx, type = 'o', ylab = "Mortality Rate", xlab = "Age (Midpoint)")
axis(2)
