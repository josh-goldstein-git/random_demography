...
```{r}
  ...
  col.names = c('generation #', 'allele type', "", "", "", "", "", "", "", "", "")) %>% kable_paper(bootstrap_options = "striped", full_width = F)
```
...
```{r}
  ...
  cbind(c(1:9), c(rep(1,9)), c(1, 1, 2, 0, 1, 3, 1, 0, 0), 
        c(1, 1, 2, 0, 1, 3, 2, 0, 0)),
  col.names = c('Allele type', "1", "2", "3"))  %>% kable_paper(bootstrap_options = "striped", full_width = F) %>% add_header_above(c(" " = 1, "Count per generation" = 3))
```
