...
## Heterogeneity slows mortality improvement 
  ...
  We take the log and the time-derivative of hazards  give @vaupel2014unobserved (Eq. 39*)
  $$ 
  \rho(x,t) = \rho_0(x,t) - \sigma^2 {d \over dt} \log
  \bar{S}_c(x,t)^{\sigma^2}
  $$ 

> Note: this formula should be correct. @vaupel2014unobserved contains a mistake: an extra minus sign in the definition of   $\bar{S}_c(x,t)$. 
...
So individual risks from one cohort to the next are going down faster it seems. Intuition?

Reversing perspective,

  $$ 
   \rho_0(x,t) = \rho(x,t) + \sigma^2 {d \over dt} \log
  \bar{S}_c(x,t)
  $$ 
...
```
So individual-level mortality progress is nearly 50% faster (1.3% vs. 0.9%) than it appears! 
...
## Hazards and Plateaus by Professor Ken Wachter
...
```{r plateaubarbi, fig.cap='Estimated plateau for cohort of Italian women from 1904. Source: @barbi2018', out.width='80%',fig.align='center',  echo=FALSE}
    knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/plateau_1.png')
    ```
...
```{r plateauhoriuchi, echo=FALSE, fig.align='center', fig.cap='Hazards across selected species. Source: @horiuchi2003', fig.height=6, out.width='80%'}
    knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/plateau_2.png')
    ```   
...
```{r echo=TRUE, fig.align='center', fig.cap='Hazards and plateau with Gamma frailty', out.width='80%', , fig.height=6}
    #Plot:
...
- Note that the x-axis shows the ages for the estimated Gompertz, that is for a population of ages 60 and above.
- $\mu_x$ appears to reach its own plateau due to the chosen parameters from the Gamma-Gompertz model. 
- In this case, we get a plateau further along than in the italian case of @barbi2018. 
- Gamma gompertz models are good at fitting plateaus mathematically but may not be aplicable with the data.  
...
- As $\beta$ increases, the individual and population hazards rise (or shift upwards). 
- For comparison, the third panel contains the $\beta=0.088$  and $k=7.045455$ values used in the previous excercise.

```{r echo=TRUE, fig.height=7, message=FALSE, fig.cap ='Changing values of beta in Gompertz hazards', fig.align='center'}
par(mfrow=c(2,3))
...
```{r echo=TRUE, fig.height=7, fig.cap = 'Changing values of k (Gamma shape parameter)', fig.align='center'}
par(mfrow=c(2,3))
...
```{r echo=TRUE, fig.cap = 'Mean frailty', fig.align='center'}
...
```{r echo=TRUE, fig.cap="$\\rho(a)$ as a function of age", fig.align='center'}
par(mfrow=c(1,2))
...
```
```{r echo=TRUE, fig.cap = 'Hazard after exposure to allele', fig.align='center'}
...
```{r echo=TRUE, fig.align='center', fig.cap="$\\rho(a)$ as a function of age", warning=FALSE}
par(mfrow=c(1,2))
