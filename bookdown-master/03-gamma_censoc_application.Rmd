...
### Calculate the variance of $z_{obs}$ to be used for estimating $\bar{z}_{obs}$. (Also plot the histogram to see if it looks gamma-like)
...
### Use extinct cohort method to estimate survivorship $\bar{S}(x)$
...
### Estimate $\hat{\bar{z}}(x)$ using the gamma-frailty result:
...
### Graph our results, showing plots for observed mortality selection and the gamma-frailty based estimate of mortality selection. Do this for several measures including  $y$, and raw (unstandardized) income.
