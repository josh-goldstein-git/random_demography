...
```{r echo=TRUE,  fig.align = 'center', fig.cap='Parents having childrens'}
...
```
```{r, echo=FALSE, fig.cap='Generation size', fig.align = 'center'}  
  matplot(t(Z.mat),
          type = "l", lty = 1, ylab = "Zn", xlab = "n")
```
- How many survive (out of 20)? log-scale
```{r echo=TRUE, fig.align = 'center', fig.cap='Generation sizes (log-scale)'}
...
```
- How would you describe the time path of the surviving lines? Let's extend the number of generations to observe any long-term trends.
```{r, fig.height = 4, fig.align = 'center'}
  ...
  for (i in 1:n_trials)
      Z.mat[i,] <- branch(n_max = n_gen)
```
```{r echo=TRUE, fig.align='center', fig.cap='Generation sizes (log-scale) over many generations'}
  suppressWarnings(matplot(t(Z.mat), log = "y", type = "l", lty = 1, ylab = "Zn", xlab = "n"))
```

- What does this remind you of? (Hint: "Leslie"). (See @harris1964theory, Figure 2, Chapter 1) 
...
- Can $d = 1$, can $d < 1$
  ...
  - If we were to find a solution less than 1.0, how would we
    interpret that?
  - Three cases
```{r, fig.align = 'center', fig.cap='PGF for a variety of z', fig.height=4}
  z = seq(0, 1.6, .01)
...
```{r echo=TRUE,  fig.align = 'center', fig.cap='Cobwed diagram (staircase)'}
  pk = c(.3, .0, .7); names(pk) <- 0:2
  z = seq(0, 1.6, .01)
  hz = pk["0"]  + pk["1"]*z + pk["2"]*z^2
  plot.fun(z,hz)
...
```{r echo=TRUE, fig.align = 'center', fig.cap='??'}
  pk = c(.3, .0, .7); names(pk) <- 0:2
  z = seq(0, 1.6, .01)
  hz = pk["0"]  + pk["1"]*z + pk["2"]*z^2
  plot.fun(z,hz)
...
```{r}
  ...
  for (i in 1:n_trials)
      Z.mat[i,] <- branch(n_max = n_gen)
```
```{r echo=TRUE,, fig.align = 'center', fig.cap='Extinction simulation'}
...
```{r echo=TRUE,  fig.align = 'center', fig.cap='Extinction simulations'}
par(mfrow = c(1,3))
...
```{r echo=TRUE,  fig.align = 'center', fig.cap='Distribution of $Z_n$'}
n <- 1:ncol(Z.mat)
plot(n, var_Zn, xlab = 'variance of Zn')
```



```{r, fig.height = 6, fig.cap='Histogram of selected $Z_n$', fig.align = 'center'}
...
```
```{r, echo=FALSE,  fig.cap = 'probability of having exactly $k$ number of girls', fig.align='center'}
plot(k, p_k)
```

Realism? See Table 10.3, p 386, @grinstead2006
...
Big payoff: the full distribution of $Z_n$ (See @grinstead2006 p. 385)
  
### A plot of Keyfitz's numbers for generations 1, 2, and 3. Is it exponential for $k > 0$?

```{r, fig.cap = 'Keyfitz’s plot for generations 1, 2, and 3', fig.align='center'}
...
5. *Simulate a critical branching process such that m = 1 by reversing the $p_1$ and $p_2$ values we’re using. Check that $m = 1$. You can use the “branch()” code in the slides.*  
    ...
    a. *See if you can do a big number of trials, 1000? For many generations, 30, 50, 100?*  
        ...
        knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/branch_simulation.png')
        ```
    b. *What fraction of lines become extinct?*  
        ...
        knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/extinctionplot.png')
        ```
    c. *What is the distribution of surviving lines? (Hint: Choose a time that is is not so distant that few lines survive)*  
        ...
        knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/generation_dist.png')
        ```
    d. *What is the mean and variance of of $Z_{10}$, $Z_{20}$ and $Z_{30}$? What will happen as $n \to \infty$?*
...
| 30 | 0.991 | 17.53045  |
        ...
        knitr::include_graphics('/hdir/0/andreamg/Year2_2019_2020/Random_demography/random_demography/bookdown-master/figures/meanvar.png')
        ```
