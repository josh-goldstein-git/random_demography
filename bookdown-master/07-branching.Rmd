...
```{r, echo= FALSE, fig.cap='Parents having childrens', fig.align = 'center'}
...
```{r, fig.height = 4}
  ...
  for (i in 1:n_trials)
      Z.mat[i,] <- branch(n_max = n_gen)
```
```{r, echo=FALSE, fig.cap='Generation sizes (log-scale) over many generations', fig.align='center'}
  suppressWarnings(matplot(t(Z.mat), log = "y", type = "l", lty = 1, ylab = "Zn", xlab = "n"))
```
...
- Can $d = 1$, can $d < 1$
  ...
  - If we were to find a solution less than 1.0, how would we
    interpret that?
  - Three cases
```{r, fig.cap='PGF for a variety of z', fig.align = 'center'}
...
```{r,  echo= FALSE, fig.cap='Cobwed diagram (staircase)', fig.align = 'center'}
  pk = c(.3, .0, .7); names(pk) <- 0:2
  z = seq(0, 1.6, .01)
  hz = pk["0"]  + pk["1"]*z + pk["2"]*z^2
  plot.fun(z,hz)
...
```{r}
  ...
  for (i in 1:n_trials)
      Z.mat[i,] <- branch(n_max = n_gen)
```
```{r, echo=FALSE, fig.cap='Extinction simulation', fig.align = 'center'}
...
```{r, echo=FALSE, fig.cap='Extinction simulations', fig.align = 'center'}
par(mfrow = c(1,3))
...
```{r, echo=FALSE, fig.cap='Distribution of $Z_n$', fig.align = 'center'}
n <- 1:ncol(Z.mat)
