...
##    z.bar[i] <- dt[age.at.death > x[i], mean(z)]
}
...
dt[, max(age.at.death), by = byear]
dt[age.at.death >= 105] ## 177 people
dt[floor(age.at.death) == 104] ## 253 people
## now we have same age range for every cohort
dt <- dt[age.at.death < 105]
dt <- dt[!is.na(incwage)] ## keep only non-missing 
...
dt[, y_both := y_educ*y_inc]
dt[, y_orig_both := y_orig_educ*y_orig_inc]
...
for (i in 1:length(x))
{
    y.bar.both[i] <- dt[age.at.death > x[i], mean(y_both)]
    y_orig.bar.both[i] <- dt[age.at.death > x[i], mean(y_orig_both)]
##    z.bar[i] <- dt[age.at.death > x[i], mean(z)]
}

par(mfrow = c(1,2))
dt[, plot(x, y_orig.bar.both)]
title("Education and wage by surviving age", cex.main = .7)
dt[, plot(x, y.bar.both)]
title("Education and wage by surviving age", cex.main = .7)
```
